...
void usage(void)
{
  fputs("Usage:  ethermidi [-d][-Fwavetable.sf2] [[ip][:port]]\n"
        ...
        "        the default port is 5000; the default ip is 0.0.0.0 [all]\n",
        stderr);
}
...
// \[ipv6-address\][:port]
// ipv4-address[:port]
// default address is 0.0.0.0 [IPV4+IPV6]
...
//  if(IPV6)   if(*szIPPort == '[') // required for ipv6 as "[ip:ad:dre:ss]:port"
//  {
//    pSA6 = (struct sockaddr_in6 *)&sa;
//#ifdef __FreeBSD__ /* Linux doesn't have this data member */
//      pSA6->sin_len = sizeof(*pSA6);
//#endif // __FreeBSD__
//  }
//  if(IPV4)
//  {
...
//        memcpy(&pSA4->sin_addr, &lTemp, sizeof(unsigned long)); // warning avoidance, should optimize ok
  ...
  pTemp = strrchr(szIPPort, ':'); // reverse look for ':'
  if(pTemp && pTemp[1])
  {
    sPort = atoi(pTemp + 1);
  }
  ...
  // for now, listen on 0.0.0.0:9000 only
  ...
  *((uint32_t *)&(pSA4->sin_addr)) = 0;
  ...
  iRval = socket(PF_INET, SOCK_DGRAM, 0);
...
//    return 1;
    ...
    iSocket = DoListenSocket(":5000");
  }
  else
  {
//  fprintf(stderr, "ip:port=\"%s\"\n", argv[0]);
//
//  if(*szWaveTable)
//  {
//    fprintf(stderr, "WAVETABLE: \"%s\"\n", szWaveTable);
//  }
...
//  {
    ...
    else if(nCmd == MIDI_SYSEX)
    {
      ...
      {
        fluid_synth_sysex (pS, pData2, nBytes2 - nBytes, NULL, NULL, NULL, 0);
